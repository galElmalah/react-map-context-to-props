...
const Id = withMemo(
  function Id({ id, setId, setCounter }) {
    amountOfRenders++;
...
describe('withMemo', () => {
  ...
  it('should pass the correct props to each component', () => {
    ...
    expect(NameWrapper.props()).toEqual({ name: 'test-name', setName: expect.any(Function) })
    expect(IdWrapper.props()).toEqual({ id: 'test-id', setId: expect.any(Function), prop: 0 })
  })

  it('should render only the relevant components ', () => {
    const wrapper = mount(<App />)
    const clickOnName = () => wrapper.find('[data-hook="name"]').simulate('click')
    const clickOnId = () => wrapper.find('[data-hook="id"]').simulate('click')
    clickOnName()
